...
export class RegistertaskComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private activitedRoute: ActivatedRoute, private router: Router, private taskService: TaskService, private authService: AuthService) {
  ...
  save(requestCreateTask: RequestCreateTask) {
      ...
      this.taskService.createTask(this.requestCreateTask).subscribe(data => {
        ...
        if (this.createdTask != null) {
          this.router.navigate(
            ["../listTask"], { relativeTo: this.activitedRoute }
          );
        }
        console.log(data);
      }),
